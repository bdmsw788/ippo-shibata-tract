...
export const RAW_DISTRICTS: RawDistrict[] = [
 {id:'honcho', name:'本庁地区', short:'本庁', areas:[
   ...
   // 以下、市の統計で世帯数が極めて少数のため秘匿され、隣接地域(緑町2丁目・新栄町2丁目)の
   // 数値に合算表記されている地域。二重計上を避けるためここでは0とする(下のコメント参照)
   ['中田町3丁目',0],['新栄町3丁目',0],['富塚(字)',0],['東塚ノ目',0],['中田(字)',0],['小舟渡',0],
 ]},
...
// 市公表の総世帯数(住民基本台帳、令和8年6月末現在)。
// 秘匿されている8地域(中田町3丁目・新栄町3丁目・富塚(字)・平山・下城・高田・小国谷・東赤谷)は
// 世帯数が極めて少数のため、市の資料でも独立した数値が公表されておらず、隣接地域
// (緑町2丁目・新栄町2丁目・住田・相馬・向中条・貝屋・滝谷)の数値に合算されている。
// これらの地域はこのアプリのリスト・地図には含めるが、世帯数は0として扱い
// (隣接地域側の数値に実数が反映済みのため)、二重計上を避けている。
export const TOTAL_HOUSEHOLDS = 38025;
