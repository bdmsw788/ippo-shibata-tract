...
export const RAW_DISTRICTS: RawDistrict[] = [
 {id:'honcho', name:'本庁地区', short:'本庁', areas:[
   ...
   // 新栄町3丁目は令和2年国勢調査の実数(179世帯、平成27年は116世帯で秘匿なし)。
   // 中田町3丁目・富塚(字)は住民基本台帳・国勢調査(平成27年/令和2年)のいずれでも
   // 世帯数非公開(秘匿)で実数が一切公表されていないため、近隣の極小地域で実際に
   // 公表されたことのある値(藤掛4・境5・小国谷5)を参考にした推定値
   ['中田町3丁目',5],['新栄町3丁目',179],['富塚(字)',5],['東塚ノ目',0],['中田(字)',0],['小舟渡',0],
 ]},
 ...
 {id:'kajikawa', name:'加治川地区', short:'加治川', areas:[
   ...
   // 平山は令和2年国勢調査の実数(7世帯)。小国谷は平成27年国勢調査の実数(5世帯、
   // 令和2年以降は秘匿)。下城・高田は住民基本台帳・国勢調査のいずれにも実数の公表がなく、
   // 近隣の極小地域の公表実績値を参考にした推定値
   ['平山',7],['下城',5],['高田',5],['小国谷',5],['金沢',0],
 ]},
 {id:'yonekura', name:'米倉地区', short:'米倉', areas:[
   ['米倉',129],['山内',78],['中々山',33],['大槻',112],
 ]},
 {id:'akatani', name:'赤谷地区', short:'赤谷', areas:[
   // 東赤谷は住民基本台帳・国勢調査のいずれにも実数の公表がなく、
   // 近隣の極小地域の公表実績値を参考にした推定値
   ['上赤谷',76],['滝谷',55],['東赤谷',5],
 ]},
...
// 住民基本台帳・国勢調査のいずれでも世帯数が秘匿(非公開)の地域については、市が公表して
// いる別の実数資料(令和2年/平成27年国勢調査 町丁目字別データ)から実数が判明したものは
// それを採用し(新栄町3丁目179・平山7・小国谷5)、判明しなかったものは近隣の極小地域の
// 公表実績を参考にした推定値(5)を採用している。そのため合計は市公表の38,025とは一致しない。
export const TOTAL_HOUSEHOLDS = 38241;
